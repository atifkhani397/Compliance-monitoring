...
def test_cs10_detection_produces_expected_alert() -> None:
    result = execute()
    assert (
        SPEC.expected_alert
        in {
...
def test_cs10_inter_agent_messages_validate_against_schema() -> None:
    result = execute()
    assert len(result["messages"]) >= 2
    assert all(message.trace_id == result["correlation_id"] for message in result["messages"])


def test_cs10_consensus_or_special_flow_is_recorded() -> None:
    result = execute()
    assert (
...
def test_cs10_escalation_and_report_types_are_correct() -> None:
    result = execute()
...
def test_cs10_audit_trail_is_complete_and_verifiable() -> None:
    result = execute()
