...
def test_cs20_detection_produces_expected_alert_or_update() -> None:
    result = execute()
    payloads = [
...
def test_cs20_inter_agent_messages_validate_against_schema() -> None:
    result = execute()
    assert len(result["messages"]) >= 2
    assert all(message.trace_id == result["correlation_id"] for message in result["messages"])


def test_cs20_consensus_or_special_flow_is_correct() -> None:
    result = execute()
    if SPEC.scenario_id == "CS-18":
        ...
        if SPEC.scenario_id == "CS-20":
            ...
            evidence_count = sum(
                len(message.payload.get("evidence_refs", []))
...
def test_cs20_escalation_and_report_types_are_correct() -> None:
    result = execute()
...
def test_cs20_audit_trail_is_complete_and_verifiable() -> None:
    result = execute()
