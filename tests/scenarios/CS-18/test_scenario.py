...
def test_cs18_detection_produces_expected_alert_or_update() -> None:
    result = execute()
    payloads = [
    ...
    if SPEC.no_alert:
        ...
        suppressed = next(
            ...
            if payload.get("update_type") == "false_positive_suppressed"
        )
        assert suppressed["changed_fields"]["confidence"] < 0.3
    else:
        assert SPEC.expected_alert in {payload.get("severity") for payload in payloads}


def test_cs18_inter_agent_messages_validate_against_schema() -> None:
    result = execute()
    assert len(result["messages"]) >= 2
    assert all(message.trace_id == result["correlation_id"] for message in result["messages"])


def test_cs18_consensus_or_special_flow_is_correct() -> None:
    result = execute()
...
def test_cs18_escalation_and_report_types_are_correct() -> None:
    result = execute()
...
def test_cs18_audit_trail_is_complete_and_verifiable() -> None:
    result = execute()
