...
def test_cs03_detection_produces_expected_alert() -> None:
    result = execute()
    assert (
        SPEC.expected_alert
        in {
...
def test_cs03_inter_agent_messages_validate_against_schema() -> None:
    result = execute()
    assert len(result["messages"]) >= 2
    assert all(message.trace_id == result["correlation_id"] for message in result["messages"])


def test_cs03_consensus_or_special_flow_is_recorded() -> None:
    result = execute()
    assert (
        result["consensus"] is not None or SPEC.special_update or SPEC.expected_conflict_type == "E"
    )
    assert result["consensus"] is not None
    assert result["consensus"].conflict_type == "B"
    assert result["consensus"].consensus_severity == "HIGH"


def test_cs03_escalation_and_report_types_are_correct() -> None:
    result = execute()
...
def test_cs03_audit_trail_is_complete_and_verifiable() -> None:
    result = execute()
